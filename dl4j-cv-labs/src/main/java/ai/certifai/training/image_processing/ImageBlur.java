...
public class ImageBlur {
    public static void main(String[] args) throws IOException {
        ...
        String myImage = new ClassPathResource("image_processing/lena.png").getFile().getAbsolutePath();

        Mat source = imread(myImage, IMREAD_GRAYSCALE);
        Mat GaussianB = new Mat();
        Mat MedianB = new Mat();

        GaussianBlur(source, GaussianB, new Size(3, 3), 2);
        medianBlur(source, MedianB, 3);

        Display.display(source, "Original");
        Display.display(GaussianB, "Gaussian Blurring");
        Display.display(MedianB, "Median Blurring");
