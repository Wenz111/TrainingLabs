...
public class ChiliGradingDetector_YOLOv2 {
...
//    private static final int nEpochs = 1;
    ...
    private static final File modelFilename = new File(System.getProperty("user.dir"), "generated-models/ChiliGradingDetector_yolov2.zip");
    ...
    private static final Scalar[] colorMap = {RED, GREEN, BLUE, YELLOW, MAGENTA, CYAN};
    private static String labelText = null;

    public static void main(String[] args) throws IOException, InterruptedException {

        // Create Iterators
        ChiliDataSetIterator.setup();
        RecordReaderDataSetIterator trainIter = ChiliDataSetIterator.trainIterator(batchSize);
        RecordReaderDataSetIterator testIter = ChiliDataSetIterator.testIterator(1);
        ...
        if (modelFilename.exists()) {
            ...
            // Model Configuration
            FineTuneConfiguration fineTuneConfiguration = getFineTuneConfiguration();

            // Modify prebuilt model's architecture
            ...
            System.out.println(model.summary(InputType.convolutional(
                    ChiliDataSetIterator.yoloHeight,
                    ChiliDataSetIterator.yoloWidth,
                    nClasses)));
                ...
                thread = new Thread(() ->
                {
                    while (frame != null) {
                        try {
                            ...
                            resize(rawImage, resizeImage, new Size(ChiliDataSetIterator.yoloWidth, ChiliDataSetIterator.yoloHeight));
    ...
    private static Mat drawResults(List<DetectedObject> objects, Mat mat, int w, int h) {
        for (DetectedObject obj : objects) {
            ...
            int x1 = (int) Math.round(w * xy1[0] / ChiliDataSetIterator.gridWidth);
            int y1 = (int) Math.round(h * xy1[1] / ChiliDataSetIterator.gridHeight);
            int x2 = (int) Math.round(w * xy2[0] / ChiliDataSetIterator.gridWidth);
            int y2 = (int) Math.round(h * xy2[1] / ChiliDataSetIterator.gridHeight);
            //Draw bounding box
            rectangle(mat, new Point(x1, y1), new Point(x2, y2), colorMap[obj.getPredictedClass()], 0, 0, 0);
            //Display label text
